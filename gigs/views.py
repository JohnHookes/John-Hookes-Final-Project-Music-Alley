...
from .forms import GigsCommentForm, GigsPostForm
from django import forms
from artists.models import Post

class GigsPage(TemplateView):
    """
    Displays home page
    """
...
class GigsList(generic.ListView):
    model = Gigs
    queryset = Gigs.objects.filter(status=1)
    template_name = "gigs/gigs.html"
    paginate_by = 6
...
def post_detail(request, slug):
    ...
    **Template:**
    ...
    queryset = Gigs.objects.filter(status=1)
    gigs = get_object_or_404(queryset, slug=slug)
    comments = gigs.comments.all().order_by("-created_on")
