...
from .models import Songs, Comment
from .forms import SongsCommentForm, SongsPostForm
from django import forms
from artists.models import Post

class SongsPage(TemplateView):
    """
    Displays home page
    """
...
class SongsList(generic.ListView):
    queryset = Songs.objects.filter(status=1)
...
def songs_detail(request, slug):
    ...
    **Template:**
    ...
    queryset = Songs.objects.filter(status=1)
    songs = get_object_or_404(queryset, slug=slug)
